document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateNameUi() {
        const label = currentId ? (projectName || 'Untitled') : 'PIXEL';
    ...
    async function renderShelf() {
        if (!els.shelfGrid || !window.PixelStore) {
            updateLibraryChrome();
            return;
        }
    ...
    function updateLibraryChrome() {
        ...
        if (els.deleteProjectBtn) els.deleteProjectBtn.classList.toggle('is-hidden', !open);
        if (els.projectNameInput) {
            els.projectNameInput.disabled = !open;
            if (!open && document.activeElement !== els.projectNameInput) {
    ...
    async function openLibrary() {
        ...
        if (els.projectNameInput && !els.projectNameInput.disabled) els.projectNameInput.focus();
    }
    ...
    async function closeLibrary() {
        if (!currentId) {
            ...
            if (items.length) {
                showToast('Open a canvas from the shelf', 'info');
                return;
            }
    ...
    async function deleteCurrent() {
        ...
        projectName = '';
        if (window.PixelStore) {
            await PixelStore.remove(id);
            await PixelStore.setCurrentId(null);
        }
        updateNameUi();
        await renderShelf();
        els.libraryRoot.classList.remove('is-hidden');
    ...
    function fillPixel(gridX, gridY, opts) {
        ...
        if (currentStep === 3 || currentTool === 'move') return;
        const pixelMap = frames[currentFrameIndex];
    ...
    function handlePointerDown(e) {
        ...
        if (currentTool === 'move') {
            pendingTap = null;
            isDrawing = false;
            beginMovePointer(cell, e.shiftKey);
            return;
        }
        isDrawing = true;
        strokeDirty = false;
    ...
    function handlePointerMove(e) {
        ...
        if (moveDrag) {
            const cell = eventToGrid(e);
            if (moveDrag.mode === 'marquee') {
                if (cell.x === moveDrag.endX && cell.y === moveDrag.endY) return;
                moveDrag.endX = cell.x;
                moveDrag.endY = cell.y;
                redrawCanvas();
                return;
            }
            const dx = cell.x - moveDrag.startX;
            const dy = cell.y - moveDrag.startY;
            if (dx === moveDrag.lastDx && dy === moveDrag.lastDy) return;
            moveDrag.lastDx = dx;
            moveDrag.lastDy = dy;
            moveDrag.pending = false;
            const result = shiftSelection(moveDrag.source, moveDrag.mask, dx, dy);
            frames[currentFrameIndex] = result.frame;
        ...
        if (currentTool === 'move') updateCanvasCursor(eventToGrid(e));
        const cell = eventToGrid(e);
    ...
    function setActiveTool(tool) {
        ...
        updateCanvasCursor();
        if (tool === 'move') maybeMoveHint();
    }

    function updateCanvasCursor(cell) {
        if (currentStep === 3) canvas.style.cursor = 'default';
        else if (currentTool === 'eyedrop') canvas.style.cursor = 'copy';
        else if (currentTool === 'move') {
            if (moveDrag && moveDrag.mode === 'move' && !moveDrag.pending) canvas.style.cursor = 'grabbing';
            else if (moveDrag && moveDrag.mode === 'marquee') canvas.style.cursor = 'crosshair';
            else if (cell && cellInSelection(cell.x, cell.y)) canvas.style.cursor = 'grab';
    ...
    function maybeMoveHint() {
        try {
            if (localStorage.getItem('pixel-move-select-hint')) return;
            localStorage.setItem('pixel-move-select-hint', '1');
        } catch (err) {
            if (maybeMoveHint.shown) return;
            maybeMoveHint.shown = true;
        }
        showToast('Tap a shape or drag a box to select, then drag to move.', 'info');
    }

    function createEmptyMask() {
        const mask = new Array(gridH);
        for (let y = 0; y < gridH; y++) mask[y] = new Array(gridW).fill(false);
        return mask;
    }
    ...
    function selectionHasCells(mask) {
        mask = mask === undefined ? selectionMask : mask;
        if (!mask) return false;
        for (let y = 0; y < gridH; y++) {
            for (let x = 0; x < gridW; x++) {
                if (mask[y] && mask[y][x]) return true;
            }
        }
        return false;
    }
    ...
    function cellInSelection(x, y) {
        return !!(selectionMask && y >= 0 && x >= 0 && selectionMask[y] && selectionMask[y][x]);
    }

    function orMask(a, b) {
        if (!a) return cloneMask(b);
        if (!b) return cloneMask(a);
        const out = createEmptyMask();
        for (let y = 0; y < gridH; y++) {
            for (let x = 0; x < gridW; x++) {
                out[y][x] = !!(a[y][x] || b[y][x]);
            }
    ...
    function floodSelect(startX, startY, frame) {
        const mask = createEmptyMask();
        if (startX < 0 || startY < 0 || startX >= gridW || startY >= gridH) return mask;
        if (!frame[startY][startX]) return mask;
        const stackX = [startX];
        const stackY = [startY];
        ...
        while (stackX.length) {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                if (mask[ny][nx] || !frame[ny][nx]) continue;
                mask[ny][nx] = true;
                stackX.push(nx);
                stackY.push(ny);
            }
    ...
    function rectSelect(x0, y0, x1, y1, frame) {
        const mask = createEmptyMask();
        const minX = Math.max(0, Math.min(x0, x1));
        const maxX = Math.min(gridW - 1, Math.max(x0, x1));
        const minY = Math.max(0, Math.min(y0, y1));
        const maxY = Math.min(gridH - 1, Math.max(y0, y1));
        if (maxX < minX || maxY < minY) return mask;
        for (let y = minY; y <= maxY; y++) {
            for (let x = minX; x <= maxX; x++) {
                if (frame[y][x]) mask[y][x] = true;
            }
        }
        return mask;
    }

    function selectAllPainted() {
    ...
    function shiftSelection(src, mask, dx, dy) {
        const next = cloneFrame(src);
        if (!mask) return { frame: next, mask: null };
        if (!dx && !dy) return { frame: next, mask: cloneMask(mask) };
        const newMask = createEmptyMask();
        const moving = [];
        for (let y = 0; y < gridH; y++) {
            for (let x = 0; x < gridW; x++) {
                if (!mask[y][x]) continue;
                moving.push({ x: x, y: y, color: src[y][x] });
                next[y][x] = null;
            }
        }
        for (let i = 0; i < moving.length; i++) {
            const nx = moving[i].x + dx;
            const ny = moving[i].y + dy;
            if (nx < 0 || ny < 0 || nx >= gridW || ny >= gridH) continue;
            next[ny][nx] = moving[i].color;
            newMask[ny][nx] = true;
        }
        return { frame: next, mask: selectionHasCells(newMask) ? newMask : null };
    }

    function beginMovePointer(cell, add) {
        const frame = frames[currentFrameIndex];
        const painted = cell.y >= 0 && cell.x >= 0 && cell.y < gridH && cell.x < gridW && frame[cell.y][cell.x];
        if (cellInSelection(cell.x, cell.y)) {
            moveDrag = {
                mode: 'move',
                startX: cell.x,
                startY: cell.y,
                ...
                lastDy: 0,
                pending: true,
                source: cloneFrame(frame),
                mask: cloneMask(selectionMask)
            };
            updateCanvasCursor(cell);
            return;
        }
        if (painted) {
            const blob = floodSelect(cell.x, cell.y, frame);
            selectionMask = add ? orMask(selectionMask, blob) : blob;
            moveDrag = {
                mode: 'move',
                startX: cell.x,
                startY: cell.y,
        ...
        moveDrag = {
            ...
            endY: cell.y,
            add: !!add,
            prevMask: add ? cloneMask(selectionMask) : null
        };
        if (!add) selectionMask = null;
        redrawCanvas();
        updateCanvasCursor(cell);
    }

    function finishMovePointer() {
        if (!moveDrag) return;
        if (moveDrag.mode === 'marquee') {
            const frame = frames[currentFrameIndex];
            let next = rectSelect(moveDrag.startX, moveDrag.startY, moveDrag.endX, moveDrag.endY, frame);
            if (moveDrag.add) next = orMask(moveDrag.prevMask, next);
            const any = selectionHasCells(next);
            selectionMask = any ? next : (moveDrag.add ? moveDrag.prevMask : null);
            const dragged = moveDrag.startX !== moveDrag.endX || moveDrag.startY !== moveDrag.endY;
            if (!any && !moveDrag.add && dragged) showToast('Nothing in that box', 'info');
            moveDrag = null;
            redrawCanvas();
            updateCanvasCursor();
            return;
        }
        const changed = !moveDrag.pending && (moveDrag.lastDx !== 0 || moveDrag.lastDy !== 0);
        moveDrag = null;
        updateCanvasCursor();
        if (changed) {
            saveToHistory();
    ...
    function nudgePixels(dx, dy) {
        if (!dx && !dy) return;
        if (currentStep === 3) return;
        if (!selectionHasCells()) return;
        const result = shiftSelection(frames[currentFrameIndex], selectionMask, dx, dy);
        frames[currentFrameIndex] = result.frame;
        selectionMask = result.mask;
        invalidateInsideMask();
        redrawCanvas();
    ...
    function cancelMoveDrag() {
        if (!moveDrag) return;
        if (moveDrag.mode === 'move' && moveDrag.source) {
            frames[currentFrameIndex] = cloneFrame(moveDrag.source);
            selectionMask = cloneMask(moveDrag.mask);
        } else if (moveDrag.mode === 'marquee') {
            selectionMask = moveDrag.add ? moveDrag.prevMask : selectionMask;
        }
        moveDrag = null;
        invalidateInsideMask();
        redrawCanvas();
    ...
    function drawSelectionOverlay() {
        if (currentTool !== 'move' || currentStep === 3) return;
        const ps = pixelSize;
        if (selectionMask) {
            ctx.save();
            ctx.fillStyle = 'rgba(58, 120, 196, 0.28)';
            for (let y = 0; y < gridH; y++) {
                for (let x = 0; x < gridW; x++) {
                    if (selectionMask[y][x]) ctx.fillRect(x * ps, y * ps, ps, ps);
                }
            }
            const t = Math.max(1, Math.round(ps * 0.14));
            ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
            for (let y = 0; y < gridH; y++) {
                for (let x = 0; x < gridW; x++) {
                    if (!selectionMask[y][x]) continue;
                    if (x === 0 || !selectionMask[y][x - 1]) ctx.fillRect(x * ps, y * ps, t, ps);
                    if (x === gridW - 1 || !selectionMask[y][x + 1]) ctx.fillRect(x * ps + ps - t, y * ps, t, ps);
                    if (y === 0 || !selectionMask[y - 1][x]) ctx.fillRect(x * ps, y * ps, ps, t);
                    if (y === gridH - 1 || !selectionMask[y + 1][x]) ctx.fillRect(x * ps, y * ps + ps - t, ps, t);
                }
            }
            ctx.restore();
        }
        if (moveDrag && moveDrag.mode === 'marquee') {
            const x0 = Math.min(moveDrag.startX, moveDrag.endX);
            const y0 = Math.min(moveDrag.startY, moveDrag.endY);
            const x1 = Math.max(moveDrag.startX, moveDrag.endX);
            const y1 = Math.max(moveDrag.startY, moveDrag.endY);
            ctx.save();
            ctx.strokeStyle = 'rgba(255, 255, 255, 0.95)';
            ctx.lineWidth = Math.max(1, Math.round(ps / 8) || 1);
            const dash = Math.max(2, Math.round(ps / 3));
            ctx.setLineDash([dash, Math.max(2, Math.round(ps / 4))]);
            ctx.strokeRect(x0 * ps + 0.5, y0 * ps + 0.5, (x1 - x0 + 1) * ps - 1, (y1 - y0 + 1) * ps - 1);
            ctx.restore();
        }
    }
    ...
    function renderFilmstrip() {
        ...
        if (frames.length < MAX_FRAMES) {
            ...
            add.addEventListener('click', function () { updateFrameCount(frames.length + 1); });
        ...
        const onThumb = strip.querySelector('.frame-thumb.on');
        if (onThumb && onThumb.scrollIntoView) {
            onThumb.scrollIntoView({ inline: 'nearest', block: 'nearest' });
        }
    }
    ...
    async function toggleAnimation() {
        ...
        if (isAnimationEnabled) {
            if (frames.length === 1) updateFrameCount(2, { stay: true });
        } else {
    ...
    function switchToStep(step) {
        ...
        updateAnimationUi();
        if (step === 1) {
            if (currentTool !== 'move') setActiveTool('draw');
            else setActiveTool('move');
            setCurrentColor(INK);
        } else if (step === 2) {
            if (currentTool === 'erase' || currentTool === 'eyedrop' || currentTool === 'move') setActiveTool(currentTool);
    ...
    function openMore() {
        ...
        const moveSheetBtn = document.getElementById('sheetMove');
        if (moveSheetBtn) moveSheetBtn.classList.toggle('is-hidden', currentStep === 3);
    ...
    window.addEventListener('mousemove', function (e) {
        if (panning || moveDrag) handlePointerMove(e);
    });
    window.addEventListener('mouseup', handlePointerUp);
    canvas.addEventListener('mouseleave', function () {
        if (!panning && !moveDrag) handlePointerUp();
    });
    const stage = els.canvasStage || canvas;
    ...
    const sheetMove = document.getElementById('sheetMove');
    if (sheetMove) {
        sheetMove.addEventListener('click', function () {
            closeMore();
            setActiveTool('move');
        });
    }
    ...
    window.addEventListener('keydown', function (e) {
        ...
        if (key === 'escape') {
            ...
            if (newCanvasResolver && newCanvasAllowCancel) {
                closeNewCanvasDialog(null);
                return;
            }
            if (currentTool === 'move' && (moveDrag || selectionHasCells())) {
                cancelMoveDrag();
                selectionMask = null;
                redrawCanvas();
                updateCanvasCursor();
        ...
        if ((e.ctrlKey || e.metaKey) && key === 'a' && currentTool === 'move' && currentStep !== 3) {
            e.preventDefault();
            selectAllPainted();
            return;
        }
        ...
        else if (key === 'v') setActiveTool('move');
        else if (key === 'f') setActiveTool('fill');
        else if (key === 'i') setActiveTool('eyedrop');
        else if (currentTool === 'move' && currentStep !== 3 && key === 'arrowleft') { e.preventDefault(); nudgePixels(-1, 0); }
        else if (currentTool === 'move' && currentStep !== 3 && key === 'arrowright') { e.preventDefault(); nudgePixels(1, 0); }
        else if (currentTool === 'move' && currentStep !== 3 && key === 'arrowup') { e.preventDefault(); nudgePixels(0, -1); }
        else if (currentTool === 'move' && currentStep !== 3 && key === 'arrowdown') { e.preventDefault(); nudgePixels(0, 1); }
        else if (key === 'g') {
            showGrid = !showGrid;
            els.grid.classList.toggle('on', showGrid);
            redrawCanvas();
