document.addEventListener('DOMContentLoaded', function () {
    ...
    function canvasSizeFor(ratioId, orientation, densityId) {
        ...
        const long = Math.min(
            MAX_GRID_SIDE,
            Math.max(short + 1, Math.round(short * Math.max(ratio.a, ratio.b) / Math.min(ratio.a, ratio.b)))
        );
    ...
    function renderNewCanvasUi() {
        ...
        if (els.densityRow) {
            ...
            DENSITIES.forEach(function (density) {
                ...
                const btn = document.createElement('button');
                btn.type = 'button';
                if (pick.density === density.id) btn.classList.add('on');
    ...
    function maybeZoomHint() {
        if (displayCellSize() >= 8) return;
        try {
            if (sessionStorage.getItem('pixel-zoom-hint')) return;
            sessionStorage.setItem('pixel-zoom-hint', '1');
        } catch (err) {
            if (zoomHintShown) return;
        }
        zoomHintShown = true;
        showToast('Pinch or scroll to zoom for detail', 'info');
    }

    function applyView() {
        canvas.style.transform = 'translate(' + viewX + 'px, ' + viewY + 'px) scale(' + viewScale + ')';
        if (els.zoomReset) {
            const idle = viewScale <= 1.02 && Math.abs(viewX) < 2 && Math.abs(viewY) < 2;
    ...
    function fillPixel(gridX, gridY) {
    ...
    function handlePointerDown(e) {
        ...
        if (isDoubleTap(pt)) {
            if (lastTap.saved) undo();
            else if (lastTap.restore && frames[currentFrameIndex][lastTap.restore.y]) {
                frames[currentFrameIndex][lastTap.restore.y][lastTap.restore.x] = lastTap.restore.prev;
                redrawCanvas();
            }
    ...
    function snapFromPicker(raw) {
        const snapped = nearestPaletteColor(raw);
        selectPaintColor(snapped);
    }
    ...
    function updateFrameCount(newCount) {
        ...
        if (newCount < oldCount) {
            ...
            for (let i = oldCount; i < newCount; i++) frames.push(createEmptyFrame(gridW, gridH));
            currentFrameIndex = newCount - 1;
        }
        renderFilmstrip();
        updateExportVisibility();
        redrawCanvas();
    ...
    async function toggleAnimation() {
        if (isAnimationEnabled && frames.length > 1) {
            const ok = await confirmAction(
                'Turn off animation?',
                'Extra frames will be removed. This cannot be undone.',
                'Turn off'
            );
            if (!ok) return;
        }
        ...
        if (isAnimationEnabled) {
            if (frames.length === 1) updateFrameCount(2);
        } else {
    ...
    function switchToStep(step) {
        ...
        if (step === 1) {
            ...
            if (isInk(currentColor)) setCurrentColor(PAPER);
            createColorPalette();
        } else {
            updateExportVisibility();
        }
    ...
    function showExportModal(title, previewEl, filename, blob, extraNote, opts) {
        ...
        overlay.appendChild(panel);
        document.body.appendChild(overlay);
        if (opts.autoDownload !== false) downloadBlob(blob, filename);
    }
    ...
    function openMore() {
        ...
        document.getElementById('sheetSaveColor').classList.toggle('is-hidden', currentStep !== 2);
        document.getElementById('sheetGrid').classList.toggle('is-hidden', currentStep === 3);
        ...
        if (sheetAnim) {
            sheetAnim.textContent = isAnimationEnabled ? 'Turn off animation' : 'Turn on animation';
        }
        ...
        if (sheetEyedrop) sheetEyedrop.classList.toggle('is-hidden', currentStep !== 2);
    ...
    stage.addEventListener('touchend', function (e) {
        e.preventDefault();
        if (e.touches.length < 2) {
            if (pinch && !pinch.moved && !pinch.cancelledStroke && Date.now() - pinch.t < 360) {
                undo();
            }
            pinch = null;
        }
